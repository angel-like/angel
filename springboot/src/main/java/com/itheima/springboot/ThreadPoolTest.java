 ...
 * 
	...
	如果此时线程池中的数量大于corePoolSize，缓冲队列workQueue满，并且线程池中的数量小于maximumPoolSize，建新的线程来处理被添加的任务。 (这个任务是新进来的，不是队列中的)
	如果此时线程池中的数量大于corePoolSize，缓冲队列workQueue满，并且线程池中的数量等于maximumPoolSize，那么通过 handler所指定的策略来处理此任务。
	也就是：处理任务的优先级为： 暂时没说
	3.核心线程corePoolSize、任务队列workQueue、最大线程maximumPoolSize，如果三者都满了，使用handler处理被拒绝的任务。
	4.当线程池中的线程数量大于 corePoolSize时，如果某线程空闲时间超过keepAliveTime，线程将被终止。这样，线程池可以动态的调整池中的线程数。
	unit可选的参数为java.util.concurrent.TimeUnit中的几个静态属性：NANOSECONDS、MICROSECONDS、MILLISECONDS、SECONDS。 
	5.workQueue常用的是：java.util.concurrent.ArrayBlockingQueue
 			ArrayBlockingQueue：数组组成的有界阻塞队列 ，默认初始值10（FIFO先进先出）。  (Queue读q）
 			LinkedBlockingQueue： 链表结构组成的有界(默认最大值Integer.Max_value)阻塞队列
 			SynchronousQueue：不存储元素的阻塞队列，也即单个元素队列(只存一个)。放进去一个就阻塞，取出来后才能继续放
	6.handler有四个选择：
...
public class ThreadPoolTest {
	...
	public static void main(String[] args) {
		...
		(3)keepAliveTime： 线程池维护 最大线程 所允许的空闲时间，当空闲时间达到keepAliveTime时，多余线程会被销毁到剩corePoolSize个为止
		(4)unit： 线程池维护线程所允许的空闲  时间的单位
		(5)workQueue： 线程池所使用的缓冲队列    -- 最后执行的几个线程   (FIFO先进先出队列、双端队列、阻塞队列)这里用的就是阻塞队列
		(5.5)ThreadFactory：表示生成线程池中工作线程的 线程工厂，用于创建线程，用默认即可 Executors.defaultThreadFactory()
		...
		ThreadPoolExecutor threadPool2 = new ThreadPoolExecutor(2, 4, 3, TimeUnit.SECONDS,
				new LinkedBlockingDeque<>(5), new ThreadPoolExecutor.AbortPolicy());
		ThreadPoolExecutor threadPool3 = new ThreadPoolExecutor(2, 4, 3, TimeUnit.SECONDS,
