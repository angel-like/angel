...
public class UserRegisteredListener2 {
	...
	public void receiveMessage(RegisterMessage registerMessage, Message message, Channel channel) {
		...
		try {
			if (userRegisterOperation(registerMessage)) {
				// 手动签收
				log.info("UserRegisteredListener2 注册监听事件，接收到消息:{} deliveryTag {}", registerMessage, deliveryTag);
	...
	private boolean userRegisterOperation(RegisterMessage registerMessage) {
		...
		if (!StringUtils.isEmpty(registerMessage.getInvitationCode())) {
			...
			if (null != userRecommendationEntity) {
				...
				log.info("[userRegisterOperation2] 上级代理状态是否正常 {}", superiorsEntity.getAgentEnable());
		...
		if (null == userAgentHierarchy) {
			log.error("[userRegisterOperation2] 默认代理等级为空 null == userAgentHierarchy");
			return false;
		}
		int retry = 0;
		while (retry < 3) {
			try {
				...
				log.error("[userRegisterOperation2] insertRecommendationCode ", e);
				retry++;
			}
		}
		if (retry >= 3) {
	...
	private UserRecommendationEntity validateRecommender(String invitationCode) {
		log.info("[userRegisterOperation2] validateRecommender {}", invitationCode);
