...
@Slf4j
@Component
public class UserRechargeListener3 {
	...
	public void receiveMessage(CallbackMessage callbackMessage, Message message, Channel channel) {
		...
		try {
			if (userRechargeOperation(callbackMessage)) {
				// 手动签收
				log.info("UserRechargeListener3  注册监听扫码充值事件，接收到消息:{} deliveryTag {}", callbackMessage, deliveryTag);
				channel.basicAck(deliveryTag, false);
				SysConstant.MESSAGE.remove(callbackMessage.getMessageId());
			} else {
				eventRetryUtil.retry(deliveryTag, callbackMessage, channel, "UserRechargeListener3");
			}
		} catch (Exception e) {
			eventRetryUtil.retry(deliveryTag, callbackMessage, channel, "UserRechargeListener3");
		}
	...
	public boolean userRechargeOperation(CallbackMessage callbackMessage) {
		...
		if (null == result) {
			log.error("[userRechargeOperation3] 该订单号对应的订单不存在 callbackMessage：{}", callbackMessage);
			return false;
		}
		if (result.getStatus() == OrderStatus.COMPLETE.getValue()) {
			log.info("[userRechargeOperation3] 该订单号对应的订单已经被确认  callbackMessage {} ", callbackMessage);
			return true;
		}
...
//				messageManagementService.save(messageManagement, result.getUserId());
				...
				log.info("[扫码充值订单确认->用户未读消息-推送消息3] message {}", message);
				pushService.pushExchange(message);
			} catch (Exception e) {
				log.error("[扫码充值订单确认,邮件和消息处理3] Exception", e);
			}
			try {
				userVipService.userUpgradeVip(result.getUserId());
			} catch (Exception e) {
				log.error("[扫码充值订单确认,vip自动升级处理3] Exception", e);
			}
		} catch (Exception e) {
			if (e instanceof DuplicateKeyException) {
				log.info("[userRechargeOperation3] 重复回调稽查订单重复");
				return true;
			}
			log.error("[userRechargeOperation3] Exception callbackMessage：{} ExceptionMsg {}", callbackMessage,
					e.getMessage());
			return false;
		}
		return true;
