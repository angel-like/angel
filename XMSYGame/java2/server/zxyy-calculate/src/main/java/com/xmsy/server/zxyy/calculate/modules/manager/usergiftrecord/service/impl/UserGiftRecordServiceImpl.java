package com.xmsy.server.zxyy.calculate.modules.manager.usergiftrecord.service.impl;
...
import com.xmsy.server.zxyy.calculate.modules.manager.usergiftrecord.dao.UserGiftRecordDao;
...
public class UserGiftRecordServiceImpl extends ServiceImpl<UserGiftRecordDao, UserGiftRecordEntity>
	...
	public void UserPropReceived(Long messageId, Long userId) {
		...
		if (messageUser.getActivityType() == Constant.ACTIVITYTYPE_1) {
			if (messageUser.getReceive()) {//判断是否领取，已经领取就跳过
				return;
			}
			// 将是否领取改成true
			messageUser.setReceive(true);
			messageUser.setStatus(true);//标为已读
			...
			UserGiftRecordEntity userGiftRecordEntity = this//道具发放记录
