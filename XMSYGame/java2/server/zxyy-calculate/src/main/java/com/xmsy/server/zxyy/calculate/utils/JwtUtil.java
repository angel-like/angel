...
public class JwtUtil {
	...
	public static String createJWT(Map<Object, Object> map) {
		// 添加构成JWT的参数
		JwtBuilder builder = Jwts.builder().setPayload(JSON.toJSONString(map)).signWith(JWT_ALG,
				generateKey(JWT_ALG, JWT_RULE)).setIssuedAt(new Date());
		// 生成JWT
	 ...
	 * @return
	 */
	public static String createJWT(String userToken) {
		if (StringUtils.isEmpty(userToken)) {
			return null;
		}
		JwtBuilder builder = Jwts.builder().signWith(JWT_ALG, generateKey(JWT_ALG, JWT_RULE)).setId(userToken).setIssuedAt(new Date());
		return builder.compact();
	}
...
//		System.out.println("JWT解密的结果：" + JwtUtil.getUserId("eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI1MDUyODIiLCJpYXQiOjE1NTUwMDg5ODd9.I0LxGmqYrP_sEM42zCOv5xBl1UVsJEfbWI8QKVM3Yfc"));
