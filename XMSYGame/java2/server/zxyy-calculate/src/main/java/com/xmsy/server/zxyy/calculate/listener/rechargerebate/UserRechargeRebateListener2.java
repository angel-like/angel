...
@Slf4j
@Component
public class UserRechargeRebateListener2 {
	...
	public void receiveMessage(RechargeRebateMessage rechargeRebateMessage, Message message, Channel channel) {
		...
		try {
			if (userRechargeRebateOperation(rechargeRebateMessage)) {
				// 手动签收
				log.info("UserRechargeRebateListener2  监听充值订单确认事件，接收到消息:{} deliveryTag {}", rechargeRebateMessage, deliveryTag);
				channel.basicAck(deliveryTag, false);
				SysConstant.MESSAGE.remove(rechargeRebateMessage.getMessageId());
			} else {
				eventRetryUtil.retry(deliveryTag, rechargeRebateMessage, channel, "UserRechargeRebateListener2");
			}
		} catch (Exception e) {
			eventRetryUtil.retry(deliveryTag, rechargeRebateMessage, channel, "UserRechargeRebateListener2");
		}
	...
	public boolean userRechargeRebateOperation(RechargeRebateMessage rechargeRebateMessage) {
		...
		if (null == result) {
			log.error("[userRechargeRebateOperation2] 该订单号对应的订单不存在 RechargeRebateMessage：{}", rechargeRebateMessage);
			return false;
		}
		if (result.getStatus() != OrderStatus.COMPLETE.getValue()) {
			log.info("[userRechargeRebateOperation2] 该订单号对应的订单未被确认  RechargeRebateMessage {} ", rechargeRebateMessage);
			return false;
		}
		try {
			...
			if(agentUser==null) {
				log.info("[userRechargeRebateOperation2] 代理商没有找到  RechargeRebateMessage {} ", rechargeRebateMessage);
				return false;
			}
			if(!agentUser.getAgentEnable()) {
				log.info("[userRechargeRebateOperation2] 代理商已经取消代理  RechargeRebateMessage {} ", rechargeRebateMessage);
				return true;
			}
			...
			if(agentHierachy==null) {
				log.info("[userRechargeRebateOperation2] 代理商层级没有找到  RechargeRebateMessage {} ", rechargeRebateMessage);
				return false;
			}
			//充值金额
			...
			try {
				...
				log.error("[userRechargeRebateOperation2] Exception rechargeRebateMessage：{} ExceptionMsg {}", rechargeRebateMessage,
						e.getMessage());
				return false;
			}
