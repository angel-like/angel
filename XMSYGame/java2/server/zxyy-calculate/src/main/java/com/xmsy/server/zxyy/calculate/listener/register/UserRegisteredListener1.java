...
@Slf4j
@Component
public class UserRegisteredListener1 {
	...
	public void receiveMessage(RegisterMessage registerMessage, Message message, Channel channel) {
		...
		try {
			if (userRegisterOperation(registerMessage)) {
				// 手动签收
				log.info("UserRegisteredListener1 注册监听事件，接收到消息:{} deliveryTag {}", registerMessage, deliveryTag);
	...
	private boolean userRegisterOperation(RegisterMessage registerMessage) {
		...
		if (null == userAgentHierarchy) {
			log.error("[userRegisterOperation1] 默认代理等级为空 null == userAgentHierarchy");
			return false;
		}
		int retry = 0;
		while (retry < 3) {
			try {
				...
				log.error("[userRegisterOperation1] insertRecommendationCode ", e);
				retry++;
			}
		}
		if (retry >= 3) {
	...
	private UserRecommendationEntity validateRecommender(String invitationCode) {
		log.info("[userRegisterOperation1] validateRecommender {}", invitationCode);
