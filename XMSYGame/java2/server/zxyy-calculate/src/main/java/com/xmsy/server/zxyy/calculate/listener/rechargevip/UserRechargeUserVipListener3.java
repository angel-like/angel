...
@Slf4j
@Component
public class UserRechargeUserVipListener3 {
	...
	public void receiveMessage(RechargeRebateMessage rechargeRebateMessage, Message message, Channel channel) {
		...
		try {
			if (userRechargeUserVipOperation(rechargeRebateMessage)) {
				// 手动签收
				log.info("UserRechargeUserVipListener3  监听充值订单确认事件，接收到消息:{} deliveryTag {}", rechargeRebateMessage, deliveryTag);
			...
			eventRetryUtil.retry(deliveryTag, rechargeRebateMessage, channel, "UserRechargeUserVipListener3");
		}
	}
	...
	public boolean userRechargeUserVipOperation(RechargeRebateMessage rechargeRebateMessage) {
		try {
			...
			if(user==null) {
				log.info("[userRechargeUserVipOperation3] 会员没有找到  RechargeRebateMessage {} ", rechargeRebateMessage);
				return false;
			}
			userVipService.userUpgradeVip(user);
		} catch (Exception e) {
			e.printStackTrace();
			log.error("[userRechargeUserVipOperation3] Exception rechargeRebateMessage：{} ExceptionMsg {}", rechargeRebateMessage,
					e.getMessage());
			return false;
		}
		return true;
