package com.xmsy.server.zxyy.webhome.modules.manager.messagemanagement.service.impl;
...
import com.xmsy.server.zxyy.webhome.common.utils.Constant.MessageContentType;
import com.xmsy.server.zxyy.webhome.common.utils.Constant.MessageTarget;
...
import com.xmsy.server.zxyy.webhome.modules.manager.sysmessagemodel.dao.SysMessageModelDao;
	...
	public boolean save(MessageManagementEntity messageManagement) {
		...
		if (accountList.length <= userList.size()) {
			...
			for (UserEntity user : userList) {
				...
				messageUser.setReceive(messageManagement.getReadonly());
		...
		throw new RRException(accountStr.replaceFirst(",", "") + ErrorCode.UserErrorCode.USER_IS_NULL.getErrMsg(),
...
//	this.baseMapper.insertEntityReturnId(messageManagement);
		...
		MessageUserEntity messageUser = new MessageUserEntity();
	    messageUser.setMessageId(Long.valueOf(0));
		...
		messageUser.setContentType(MessageContentType.USER_CONTENT.getValue());
		messageUser.setTitle(messageModel.getTitle());
		messageUser.setContent(messageModel.getContent());
		messageUser.setEffectTime(EFFECT_TIME);
		messageUser.setFailureTime(FAIL_TIME);
		messageUser.setReadonly(messageModel.getReadonly());
		messageUser.setActivityId(activityId);
	    messageUserDao.insertEntityReturnId(messageUser);
		...
		if(propList!=null && !propList.isEmpty()) {
			...
			for(SysMessageModelPropEntity prop : propList) {
				...
				messageProp.setMessageId(messageUser.getId());
