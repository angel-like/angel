package com.xmsy.server.zxyy.webhome.modules.manager.webhomepromotions.controller;
...
public class WebhomePromotionsController {
    ...
    public R list(WebhomePromotionsEntity webhomepromotions, PageParam pageParam){
        ...
        Page<WebhomePromotionsEntity> result = new Page<WebhomePromotionsEntity>(pageParam.getPage(), pageParam.getLimit());
		...
		if(!CollectionUtils.isEmpty(result.getRecords())) {
			for(WebhomePromotionsEntity entity:result.getRecords()) {
				if(entity.getTypeId()!=null||entity.getTypeId()!=0) {
					WebhomePromotionsTypeEntity promotionsType=webhomePromotionsTypeService.selectById(entity.getTypeId());
