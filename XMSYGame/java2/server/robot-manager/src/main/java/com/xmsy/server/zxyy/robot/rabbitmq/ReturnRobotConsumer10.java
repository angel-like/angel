...
@Slf4j
@Component
public class ReturnRobotConsumer10 {
	@Autowired
	...
	public void receiveMessage(String receiveMessage, Message message, Channel channel) {
		...
		try {
			...
			try {
				// 失败重试
				log.info("ReturnRobotConsumer10 接收到消息/开始回收机器人:{} deliveryTag {}", receiveMessage, deliveryTag);
				...
				if (null == retry) {
					...
					log.error("ReturnRobotConsumer10 接收到消息之后重试5次仍然失败.receiveMessage {}", receiveMessage);
					return;
				} else {
					channel.basicReject(deliveryTag, true);
				...
				log.error("ReturnRobotConsumer10 签收异常.", e1);
			}
		}
