...
@Slf4j
@Component
public class ReturnRobotConsumer8 {
	@Autowired
	...
	public void receiveMessage(String receiveMessage, Message message, Channel channel) {
		...
		try {
			...
			try {
				log.info("ReturnRobotConsumer8 接收到消息/开始回收机器人:{} deliveryTag {}", receiveMessage, deliveryTag);
				// 失败重试
				...
				if (null == retry) {
					...
					log.error("ReturnRobotConsumer8 接收到消息之后重试5次仍然失败.receiveMessage {}", receiveMessage);
					return;
				} else {
					channel.basicReject(deliveryTag, true);
				...
				log.error("ReturnRobotConsumer8 签收异常.", e1);
			}
		}
