package com.xmsy.server.zxyy.manager.modules.manager.cashqrcode.controller;
...
public class CashQrcodeController {
    ...
    public R enable(@RequestBody Long[] ids) {
        for (Long id : ids) {
            CashQrcodeEntity cashQrcodeEntity = new CashQrcodeEntity();
            cashQrcodeEntity.setId(id);
            cashQrcodeEntity.setEnable(SysConstant.ENABLE_TRUE);
            cashQrcodeService.updateById(cashQrcodeEntity);
        }
        return R.ok();
    }
    ...
    public R prohibit(@RequestBody Long[] ids) {
        for (Long id : ids) {
            CashQrcodeEntity cashQrcodeEntity = new CashQrcodeEntity();
            cashQrcodeEntity.setId(id);
            cashQrcodeEntity.setEnable(SysConstant.ENABLE_FALSE);
            cashQrcodeService.updateById(cashQrcodeEntity);
        }
        return R.ok();
    }
