...
public class LoginRepeatAspect {
	...
	@Pointcut("@annotation(com.xmsy.server.zxyy.manager.common.annotation.LoginRepeat)")
	...
	public Object around(ProceedingJoinPoint point) throws Throwable {
		...
		if (args[0] instanceof LoginParams) {
			...
			log.info("[APP登入操作获取锁] lockKey {} lockResult {}", lockKey, lockResult);
		} else {
			...
			lockKey = stringBuilder.toString();
			lockResult = redisLockUtil.setLock(lockKey, lockKey);
			log.info("[web登入操作获取锁] lockKey {} lockResult {}", lockKey, lockResult);
		}
		Object result = null;
		if (lockResult) {
			try {
