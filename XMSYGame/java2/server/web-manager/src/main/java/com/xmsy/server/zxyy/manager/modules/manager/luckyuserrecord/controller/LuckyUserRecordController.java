package com.xmsy.server.zxyy.manager.modules.manager.luckyuserrecord.controller;
...
import com.xmsy.server.zxyy.manager.modules.manager.luckyuserrecord.entity.LuckyUserRecordEntity;
import com.xmsy.server.zxyy.manager.modules.manager.luckyuserrecord.service.LuckyUserRecordService;
...
public class LuckyUserRecordController {
    ...
    public R list(LuckyUserRecordEntity luckyuserrecord, PageParam pageParam){
		...
		Wrapper<LuckyUserRecordEntity> entityWrapper = new EntityWrapper<LuckyUserRecordEntity>(luckyuserrecord).gt("user_id",0l);
        entityWrapper.orderBy("create_time", false);
		entityWrapper.orderBy(pageParam.getSort(), pageParam.getDirection());

        if(luckyuserrecord.getSTime()!=null) {
            entityWrapper.gt("create_time", luckyuserrecord.getSTime());
            if(luckyuserrecord.getETime()!=null) {
                entityWrapper.le("create_time", luckyuserrecord.getETime());
            }
        }
