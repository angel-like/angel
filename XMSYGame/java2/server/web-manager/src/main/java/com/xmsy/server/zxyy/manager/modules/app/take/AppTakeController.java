package com.xmsy.server.zxyy.manager.modules.app.take;
...
import com.xmsy.server.zxyy.manager.modules.manager.cashpriceconfig.entity.CashPriceConfigEntity;
import com.xmsy.server.zxyy.manager.modules.manager.cashpriceconfig.service.CashPriceConfigService;
...
//		}
	...
	public R saveTakeCommission(@RequestBody @Valid OrderTakeMoneyEntity takeMoneyRecordEntity,
		...
		try {
			...
			message = new UserInfoMessage(pushMessage, null);
			log.info("[用户佣金取款-推送消息] message {}", message);
			pushService.pushExchange(message);
		} catch (Exception e) {
