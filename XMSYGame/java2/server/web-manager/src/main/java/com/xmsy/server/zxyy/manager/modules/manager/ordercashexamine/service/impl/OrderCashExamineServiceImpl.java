package com.xmsy.server.zxyy.manager.modules.manager.ordercashexamine.service.impl;
...
public class OrderCashExamineServiceImpl extends ServiceImpl<OrderCashExamineDao, OrderCashExamineEntity>
	...
	public BigDecimal saveCashExamine(OrderRechargeEntity orderRechargeEntity, UserEntity user) throws Exception {
		...
		if (null == hierarchyEntity) {
			...
			throw new RRException(ErrorCode.HierarchyErrorCode.HIERARCHY_ISNULL_ERRO.getErrMsg(),
					ErrorCode.HierarchyErrorCode.HIERARCHY_ISNULL_ERRO.getCode());
		}
		// 结算上次稽查记录
...
//		}
	 ...
	 */
	@Override
	public BigDecimal saveCashExamine(OrderRechargeEntity orderRechargeEntity, UserEntity user
			,BigDecimal discountMultiple
			,BigDecimal rechargeMultiple) throws Exception {
		// 本次充值需要满足的打码量
		BigDecimal userNeedBetResult = BigDecimal.ZERO;
		Long hierarchyId = user.getHierarchyId();
		UserHierarchyEntity hierarchyEntity = userHierarchyDao.selectById(hierarchyId);
		...
		orderCashExamineEntity.setHierarchyDiscountAmount(discountAmount);
		// 层级优惠打码倍数
		orderCashExamineEntity.setHierarchyDiscountMultiple(discountMultiple);
		// 层级优惠打码
		BigDecimal hierarchyDiscountBet = discountAmount.multiply(discountMultiple);
		orderCashExamineEntity.setHierarchyDiscountBet(hierarchyDiscountBet);
		// 层级常态打码倍数
		orderCashExamineEntity.setHierarchyNormalMultiple(rechargeMultiple);
		// 层级常态打码
		BigDecimal hierarchyNormalBet = new BigDecimal(orderRechargeEntity.getAmount())
				.multiply(rechargeMultiple);
		orderCashExamineEntity.setHierarchyNormalBet(hierarchyNormalBet);
		// 层级放宽打码比例
		orderCashExamineEntity.setHierarchyRelaxationRate(hierarchyEntity.getRelaxationRate());
		// 层级放宽打码： （层级优惠打码+层级常态打码)*层级放宽打码比例
		BigDecimal hierarchyRelaxationBet = (hierarchyDiscountBet.add(hierarchyNormalBet))
				.multiply(hierarchyEntity.getRelaxationRate());
		orderCashExamineEntity.setHierarchyRelaxationBet(hierarchyRelaxationBet);
		// 层级行政费率
		orderCashExamineEntity.setHierarchyAdministrativeRate(hierarchyEntity.getAdministrativeRate());
		// 总需要打码： （层级优惠打码+层级常态打码-层级放宽打码)
	...
	@Override
	public boolean updateCashBet(Long userId, BigDecimal userValidBet) {
		return this.baseMapper.updateCashBet(userId,userValidBet);
	}

	@Override
	public boolean updateValidCashBet(Long userId) {
		return this.baseMapper.updateValidCashBet(userId);
	}

	@Override
	public boolean updateCashBetAndTime(Long userId, BigDecimal userValidBet, Date updateTime) {
		return this.baseMapper.updateCashBetAndTime(userId,userValidBet,updateTime);
	}
	...
	public long getUserValidBet(Long userId, Date orderTime) {
		// 查询用户的有效下注
	...
	public OrderCashExamineEntity updateCashExamineForRecharge(UserEntity user, UserHierarchyEntity hierarchyEntity,
		...
		while (i < 10) {
			...
			BigDecimal userValidBetDecimal = new BigDecimal(userValidBet == null ? 0 : userValidBet)
					.divide(new BigDecimal(Constant.COIN_RATE)).add(orderCashExamineEntity.getUserValidBet());
			...
			if (updateById(orderCashExamineEntity)) {
				...
				orderCashExamine.setId(orderCashExamineEntity.getId());
	...
	@Override
	public boolean bindUserinfoGiftCashExamine(UserEntity user, Long coin, BigDecimal codeMultiple) throws Exception {
		...
		// 优惠金额
		orderCashExamineEntity.setHierarchyDiscountAmount(BigDecimal.ZERO);
		// 层级优惠打码倍数
		orderCashExamineEntity.setHierarchyDiscountMultiple(BigDecimal.ZERO);
		// 层级优惠打码
		orderCashExamineEntity.setHierarchyDiscountBet(BigDecimal.ZERO);
		// 层级常态打码倍数
		BigDecimal hierarchyNormalMultiple = codeMultiple;
		orderCashExamineEntity.setHierarchyNormalMultiple(hierarchyNormalMultiple);
		// 层级常态打码
		BigDecimal hierarchyNormalBet = amount.multiply(hierarchyNormalMultiple);
		orderCashExamineEntity.setHierarchyNormalBet(hierarchyNormalBet);
		// 层级放宽打码比例
		orderCashExamineEntity.setHierarchyRelaxationRate(BigDecimal.ZERO);
		// 层级放宽打码： （层级优惠打码+层级常态打码)*层级放宽打码比例
		orderCashExamineEntity.setHierarchyRelaxationBet(BigDecimal.ZERO);
		// 层级行政费率
		orderCashExamineEntity.setHierarchyAdministrativeRate(BigDecimal.ZERO);
		// 总需要打码： （层级优惠打码+层级常态打码+上次缺少的打码量-上次剩余打码-层级放宽打码)
		BigDecimal userNeedBet = hierarchyNormalBet.compareTo(BigDecimal.ZERO) < 0 ? BigDecimal.ZERO
				: hierarchyNormalBet;
		// 金币加余额
		BigDecimal validMoney = amount.add(user.getMoney());
		// 上条稽查记录
		if (null != preOrderCashExamineEntity) {
	...
	@Override
	public BigDecimal countUserNeedBet(BigDecimal amount, Long hierarchyId) {
		// 
		UserHierarchyEntity hierarchyEntity = userHierarchyDao.selectById(hierarchyId);
		if (null == hierarchyEntity) {
			log.error("[countUserNeedBet] 用户等级为空 ");
			throw new RRException(ErrorCode.HierarchyErrorCode.HIERARCHY_ISNULL_ERRO.getErrMsg(),
					ErrorCode.HierarchyErrorCode.HIERARCHY_ISNULL_ERRO.getCode());
		}
		BigDecimal discountAmount =BigDecimal.ZERO;
		...
		BigDecimal hierarchyRelaxationBet = (hierarchyDiscountBet.add(hierarchyNormalBet))
				.multiply(hierarchyRelaxationRate);
		// 本次充值需要满足的打码量： （层级优惠打码+层级常态打码-层级放宽打码)
		BigDecimal userNeedBet = hierarchyDiscountBet.add(hierarchyNormalBet).subtract(hierarchyRelaxationBet);
		userNeedBet = userNeedBet.compareTo(BigDecimal.ZERO) < 0 ? BigDecimal.ZERO : userNeedBet;
		return userNeedBet;
	}
