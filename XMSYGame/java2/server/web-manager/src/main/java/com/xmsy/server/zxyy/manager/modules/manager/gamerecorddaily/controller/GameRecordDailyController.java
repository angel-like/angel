package com.xmsy.server.zxyy.manager.modules.manager.gamerecorddaily.controller;
...
public class GameRecordDailyController {
     ...
     * 获取抽水列表
     */
    @RequestMapping("/userPumpList")
    @RequiresPermissions("gamerecorddaily:gamerecorddaily:userPumpList")
    public R userPumpList(GameRecordDailyEntity gamerecorddaily, PageParam pageParam){
    	List<GameRecordDailyEntity> selectuserPumpList = gameRecordDailyService.selectuserPumpList(gamerecorddaily);
    	BigDecimal waterProfitSum = BigDecimal.ZERO;
    	for(GameRecordDailyEntity data:selectuserPumpList) {
    		waterProfitSum = waterProfitSum.add(MathUtil.getBigDecimal(data.getWaterProfit()));
    	}
    	return R.ok().put("page", gameRecordDailyService.selectuserPumpTotal(gamerecorddaily,pageParam)).
    			put("waterProfitSum", waterProfitSum);
    }
