package com.xmsy.network.pay.xinfapay.callback;
...
@Slf4j
@Controller
public class XinfaCallbackHandler extends BaseCallbackHandler {

	public XinfaCallbackHandler() {
	...
	protected CallbackResult getCallbackResult(HttpServletRequest request) {
		...
		try {
			...
			log.info("[xinfaPay->getCallbackResult] data {}", data);
			...
			log.error("[xinfaPay] parameters {}", data, e);
			return CallbackResult.err();
		}
	}
	...
	protected void getResponse(boolean orderHandleResult, HttpServletResponse response) {
		if (orderHandleResult) {
			try {
				...
				log.error("[xinfaPay->getResponse] UnsupportedEncodingException ", e);
			} catch (IOException e) {
				log.error("[xinfaPay->getResponse] IOException ", e);
			}
			return;
		}
		return;
