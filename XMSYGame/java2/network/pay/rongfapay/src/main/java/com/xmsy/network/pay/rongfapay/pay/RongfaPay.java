package com.xmsy.network.pay.rongfapay.pay;
...
import com.xmsy.network.pay.rongfapay.config.RongfapayConfig;
import com.xmsy.network.pay.rongfapay.def.Config;
import com.xmsy.network.pay.rongfapay.utils.ResultUtil;
import com.xmsy.network.pay.rongfapay.utils.ToolKit;
...
public class RongfaPay extends PayServiceBase {

	private final static String title = "零用品";
	private final static String encodeError = "支付异常请重试";

	@Resource
	private RongfapayConfig rongfapayConfig;
	...
	protected GlobalResult<ResultData> doPay(PayParam param) {
		...
		if (PayPlatform.PC == param.getPlatform()) {
			metaSignMap.put("netwayType", Config.PAY_CHANNEL_PC.get(param.getPayChannel()));
		}
		if (PayPlatform.H5 == param.getPlatform()) {
			metaSignMap.put("netwayType", Config.PAY_CHANNEL_H5.get(param.getPayChannel()));
		}
		metaSignMap.put("amount", String.valueOf(param.getAmount() * 100));//
		metaSignMap.put("goodsName", title);//
		metaSignMap.put("notifyUrl", rongfapayConfig.getCallbackUrl());// 回调地址
		metaSignMap.put("notifyViewUrl", rongfapayConfig.getCallbackUrl());// 回显地址
		...
		try {
			...
			String reqParam = "data=" + URLEncoder.encode(paramStr, ToolKit.CHARSET) + "&merchNo="
					+ metaSignMap.get("merchNo") + "&version=V3.6.0.0";
			...
			if (!ResultUtil.success(jsonResult.getString("stateCode"))) {
				log.error("[rongfapay] 请求支付失败  result {}", resultJsonStr);
			...
			if (targetString.equals(resultSign)) {
				...
				log.error("[rongfapay] 请求支付返回验签失败  result {}", resultJsonStr);
				return ResultUtils.getErrorResult(jsonResult.getString("msg"));
			}
			...
			log.info("[rongfapay]->HttpUtil.post requestParam {} result {}", reqParam, resultJsonStr);
	...
	public static void main(String[] args) {
		PayParam param = new PayParam().setAmount(20).setPayChannel("weixin").setOrderIp("192.168.0.166")
				...
				.setOrderNotifyUrl("http://zxyy-hsk.qicp.io:11312/V1.0/pay/rongfapay/payCallback");
		log.info("[rongfapay] PayParam {}", param);
		System.out.println(new RongfaPay().doPay(param));
	...
	protected void init(SysConfigMessage commonConfig,String callbackUrl) {
		...
		PayConfigUtils.setPayConfig(commonConfig, rongfapayConfig,callbackUrl);
		Config.APPID = rongfapayConfig.getUid();
		Config.APPSECRET = rongfapayConfig.getSecret();
		Config.PUBLIC_KEY = rongfapayConfig.getPublicKey();
		Config.PRIVATE_KEY = rongfapayConfig.getPrivateKey();
		Config.ORDER_URL = rongfapayConfig.getOrderUrl();
		if (null != rongfapayConfig.getProduct()) {
			Config.PAY_CHANNEL_H5 = rongfapayConfig.getProduct().get(PayPlatform.H5);
			Config.PAY_CHANNEL_PC = rongfapayConfig.getProduct().get(PayPlatform.PC);
		}
		Config.PRIVATE_KEY = rongfapayConfig.getPrivateKey();
		Config.PUBLIC_KEY = rongfapayConfig.getPublicKey();
		log.info("支付公司 {} 配置初始化 config {}", getName(), rongfapayConfig);
	}
