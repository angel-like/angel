package com.xmsy.network.pay.rongfapay.callback;
...
@Slf4j
@Controller
public class RongfaCallbackHandler extends BaseCallbackHandler {

	public RongfaCallbackHandler() {
	...
	protected CallbackResult getCallbackResult(HttpServletRequest request) {
		...
		try {
			...
			log.info("[rongfapay->getCallbackResult] data {}", data);
			...
			log.error("[rongfapay] parameters {}", data, e);
			return CallbackResult.err();
		}
	}
	...
	protected void getResponse(boolean orderHandleResult, HttpServletResponse response) {
		if (orderHandleResult) {
			try {
				...
				log.error("[rongfapay->getResponse] UnsupportedEncodingException ", e);
			} catch (IOException e) {
				log.error("[rongfapay->getResponse] IOException ", e);
			}
			return;
		}
		return;
