  ...
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @submit.native.prevent @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
		...
		<div style="color:red">点击菜单名称可进入下一级菜单</div>
    <el-table
      :data="dataList"
      border
      ...
      <el-table-column
        ...
        label="菜单名称">
				<template slot-scope="scope">
					<el-button  type="text" size="small" @click="nextLevelHandle(scope.row.id)">{{scope.row.name}}</el-button>
				</template>
      </el-table-column>
			<el-table-column
				prop="type"
...
<script>
  import AddOrUpdate from './webhomemenu-add-or-update'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/webhomemenu/webhomemenu/list'),
          method: 'get',
      ...
      nextLevelHandle (parentId) {
       this.$router.push({path:'/webhomemenuchildren',query:{parentId:parentId}})
      },
			//绑定回车事件
			keyupSubmit(){
