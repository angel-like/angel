<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
			<el-tooltip class="item" effect="dark" content="游戏名" placement="top-start">
    <el-form-item label="标题" prop="title">
      <el-input v-model="dataForm.title" placeholder="标题"></el-input>
    </el-form-item>
		</el-tooltip>
		<el-tooltip class="item" effect="dark" content="游戏介绍" placement="top-start">
   ...
   <el-tooltip class="item" effect="dark" content="游戏下载图片" placement="top">
   ...
   :file-list="fileList" :data="form" style="text-align: center;">
	...
	<!-- <div style="color: red;">上传的图片尺寸为：345*366   </div> -->
  ...
  export default {
    ...
    methods: {
      init (id) {
				 //下拉获取图片尺寸
				this.$http({
					url: this.$http.adornUrl(`/picturesize/picturesize/select/0/10`),
					method: 'get',
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/imgameintroduction/imgameintroduction/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 200) {
                this.dataForm.title = data.imgameintroduction.title
                this.dataForm.content = data.imgameintroduction.content
                this.dataForm.enclosureId = data.imgameintroduction.enclosureId
                this.dataForm.enable = data.imgameintroduction.enable
                this.dataForm.orderNo = data.imgameintroduction.orderNo
      ...
      dataFormSubmit () {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/imgameintroduction/imgameintroduction/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
