      ...
      <el-form-item>
       <el-select v-model="dataForm.typeId" placeholder="活动类型" clearable>
       	<el-option
			...
			<el-form-item>
			<el-input v-model="dataForm.remake" placeholder="备用字段" clearable></el-input>
			</el-form-item>
      <el-form-item>
        <el-button @click="getDataListQuery()">查询</el-button>
        <el-button v-if="isAuth('webhomepromotions:webhomepromotions:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
        <el-button v-if="isAuth('webhomepromotions:webhomepromotions:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
      </el-form-item>
    </el-form>
    <el-table
      :data="dataList"
...
<script>
  import AddOrUpdate from './webhomepromotions-add-or-update'
  export default {
    data () {
      return {
        dataForm: {
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          url: this.$http.adornUrl('/webhomepromotions/webhomepromotions/list'),
          method: 'get',
          params: this.$http.adornParams({
            'page': this.pageIndex,
            'limit': this.pageSize,
      ...
      deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/webhomepromotions/webhomepromotions/delete'),
            method: 'post',
