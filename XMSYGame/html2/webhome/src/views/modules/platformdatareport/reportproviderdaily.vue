<template>
  <div class="mod-config">
    <el-form :inline="true" :model="dataForm" @submit.native.prevent @keyup.enter.native="getDataList()">
      ...
      <el-form-item>
        <el-select v-model="dataForm.providerCode" placeholder="游戏平台" clearable>
          <el-option
    ...
    <el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="width: 100%;">
      ...
      <el-table-column prop="providerCode" header-align="center" align="center" label="平台名称">
        <template slot-scope="scope">
          <div v-if="scope.row.providerCode==0">
            自营游戏
          </div>
          <div v-if="scope.row.providerCode==1">
            开元游戏
          </div>
          <div v-if="scope.row.providerCode==2">
            ig视讯
          </div>
        </template>
      </el-table-column>
  ...
  export default {
    ...
    methods: {
      getDataList() {
        ...
        this.$http({
          url: this.$http.adornUrl('/reportproviderdaily/reportproviderdaily/list'),
          method: 'get',
          params: this.$http.adornParams({
            'page': this.pageIndex,
            'limit': this.pageSize,
            'sort': 'count_day,provider_code',
            'direction': false,
            'startTime': startTime,
            'endTime': endTime,
            'providerCode': this.dataForm.providerCode,
          })
        }).then(({
                   data
                 }) => {
