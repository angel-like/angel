	...
	<div class="mod-configg">
		...
		<el-table  
          ...
          style="width: 100%;">
			...
			<el-table-column prop="rechargeTotal" header-align="center" align="center" label="当前充值/兑换">
				<template slot-scope="scope">
					<div v-if="scope.row.rechargeTotal != null">充值总额:{{scope.row.rechargeTotal}}</div>
					<div v-if="scope.row.totalExchangeAmount!=null&&scope.row.totalExchangeAmount>0">兑换总额:{{scope.row.totalExchangeAmount/100}}</div>
					<div v-if="scope.row.totalExchangeAmount==null||scope.row.totalExchangeAmount==0">兑换总额:0</div>
					<div v-if="scope.row.investmentTotal != null">营收总额:{{(scope.row.investmentTotal-scope.row.outputTotal)/100}}</div>
				</template>
			</el-table-column>
			<el-table-column prop="rechargeTotal" header-align="center" align="center" label="当前用户">
				<template slot-scope="scope">
					<div>注册:{{scope.row.registereNum}}&nbsp;&nbsp;&nbsp;赢钱:{{scope.row.winNum}}&nbsp;&nbsp;&nbsp;充值人数:{{scope.row.rechargeNum}}</div>
					<div>活跃:{{scope.row.activeNum}}&nbsp;&nbsp;&nbsp;
					输钱:{{scope.row.loseNum}}&nbsp;&nbsp;&nbsp;
					兑换人数:
					<span v-if="scope.row.exchangeNum!=null&&scope.row.exchangeNum>0">{{scope.row.exchangeNum}}</span>
					<span v-if="scope.row.exchangeNum==null||scope.row.exchangeNum==0">0</span></div>
				</template>
			</el-table-column>
