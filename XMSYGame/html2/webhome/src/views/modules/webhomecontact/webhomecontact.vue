	...
	<div class="mod-config">
		<el-form :inline="true" :model="dataForm" @submit.native.prevent @keyup.enter.native="getDataList()">
			...
			<el-form-item>
				...
				<el-button v-if="isAuth('webhomecontact:webhomecontact:save')" type="primary" @click="addOrUpdateHandle()">新增</el-button>
				<el-button v-if="isAuth('webhomecontact:webhomecontact:delete')" type="danger" @click="deleteHandle()" :disabled="dataListSelections.length <= 0">批量删除</el-button>
			</el-form-item>
		</el-form>
		<el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="width: 100%;">
			...
			<el-table-column prop="type" header-align="center" align="center" label="类型(联系方式，网站信息）">
				<template slot-scope="scope">
					<div v-if="scope.row.type=='contact'">
						联系信息
					</div>
					<div v-if="scope.row.type=='website'">
						网站信息
					</div>
				</template>
			</el-table-column>
			...
			<el-table-column prop="enable" header-align="center" align="center" label="是否展示">
				<template slot-scope="scope">
...
<script>
	import AddOrUpdate from './webhomecontact-add-or-update'
	export default {
		data() {
			return {
				dataForm: {
		...
		methods: {
			...
			getDataList() {
				...
				this.$http({
					url: this.$http.adornUrl('/webhomecontact/webhomecontact/list'),
					method: 'get',
			...
			deleteHandle(id) {
				...
				this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
					...
					this.$http({
						url: this.$http.adornUrl('/webhomecontact/webhomecontact/delete'),
						method: 'post',
