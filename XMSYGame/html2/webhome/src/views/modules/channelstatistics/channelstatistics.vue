<template>
		...
		<el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle" style="width: 100%;">
			...
			<el-table-column prop="userNum" header-align="center" align="center" label="会员数量">
				<template slot-scope="scope">
					<div else>
						<a @click="readUserNum(scope.row.channelCode,scope.row.startTime,scope.row.endTime)" title="查看详情" style="cursor:pointer ">{{scope.row.userNum}}</a>
					</div>
				</template>
			</el-table-column>
			<el-table-column prop="validBet" header-align="center" align="center" label="打码量">
					<template slot-scope="scope">
						<div>{{scope.row.validBet/100}}</div>
					</template>
			</el-table-column>
    ...
    <add-or-update v-if="addOrUpdateVisible" ref="addOrUpdate" @refreshDataList="getDataList"></add-or-update>-->
		...
		<user-num v-if="userNumVisible" ref="userNum" @refreshDataList="getDataList"></user-num>
	</div>
</template>
<script>
	//import AddOrUpdate from './adchannelconfig-add-or-update'
	import adChannelConfig from './read_adchannelconfig'
	import userNum from './read_usernum'
	...
	export default {
		...
		methods: {
			...
			//通过渠道代码读取会员数量
			readUserNum(channelCode,startTime,endTime){
				this.userNumVisible = true
				//console.log("开始时间:"+startTime+"结束时间："+endTime)
				this.$nextTick(() => {
					this.$refs.userNum.init(channelCode,startTime,endTime)
				})
			},
				//绑定回车事件
			keyupSubmit(){
