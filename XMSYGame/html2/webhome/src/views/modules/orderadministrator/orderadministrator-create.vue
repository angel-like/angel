  ...
  <div class="mod-config">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
      ...
      <el-form-item label="充值类型" prop="rechargeType">
        <el-select v-model="dataForm.rechargeType"  placeholder="充值类型" @change="clear(dataForm.rechargeType)" clearable>
          <el-option v-for="item in typeOptions" :key="item.code" :label="item.name" :value="item.code+','+item.name">
          </el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item v-if="dataForm.rechargeType.indexOf('0,')>-1" label="操作金额(元)" prop="amount">
        <el-input type="number" style="width: 150px" @change="getDiscount()" v-model="dataForm.amount"
                  placeholder="金额"></el-input>
      </el-form-item>
      <el-form-item label="存入金额打码倍数" prop="rechargeMultiple" v-if="dataForm.operationType==0&&dataForm.rechargeType.indexOf('0,')>-1">
        <el-input-number v-model="dataForm.rechargeMultiple" :precision="2" :step="1" :min="0"
                         @change="countCodingCode" placeholder="存入金额打码倍数"></el-input-number>
      </el-form-item>
      <el-form-item label="优惠金额(元)" prop="discountAmount" v-if="dataForm.operationType==0&&dataForm.rechargeType.indexOf('0,')>-1">
        <el-input type="number" style="width: 150px" v-model="dataForm.discountAmount" readonly
                  placeholder="优惠金额"></el-input>
        <el-switch v-model="disSwitch"  active-color="#13ce66" inactive-color="#ff4949">
        </el-switch>
      </el-form-item>
      <el-form-item label="优惠金额(元)" prop="discountAmount" v-if="dataForm.operationType==0&&dataForm.rechargeType.indexOf('0,')==-1">
        <el-input type="number" style="width: 200px" v-model="dataForm.discountAmount" @change="calculation"
                  placeholder="优惠金额"></el-input>
      </el-form-item>
      <el-form-item label="VIP优惠金额(元)" prop="vipDiscount" v-if="dataForm.operationType==0&&dataForm.rechargeType.indexOf('0,')>-1">
        <el-input type="number" style="width: 150px" v-model="dataForm.vipDiscount" readonly
                  placeholder="VIP优惠金额"></el-input>
      </el-form-item>
      <el-form-item label="优惠金额打码倍数" prop="discountMultiple" v-if="dataForm.operationType==0">
        <el-input-number v-model="dataForm.discountMultiple" :precision="2" :step="1" :min="0" @change="countCoinNum"
                         placeholder="优惠金额打码倍数"></el-input-number>
      </el-form-item>
...
//           ],
    ...
    activated() {
      this.$http({
        url: this.$http.adornUrl(`/sysdictionary/sysdictionary/select/rechargeType`),
        method: 'get',
      ...
      this.$nextTick(() => {
        ...
        this.dataForm.rechargeType = '0,人工存入'
...
// 					});
    ...
    methods: {
      ...
      countCoinCode () { // 输入金额触发的
        ...
        this.codingCoin = (Number(this.dataForm.discountAmount) + Number(this.dataForm.vipDiscount)) * this.dataForm.discountMultiple +
      ...
      countCoinNum (currentValue, oldValue) { // 优惠金额打码倍数触发
        ...
        this.codingCoin = (Number(this.dataForm.discountAmount) + Number(this.dataForm.vipDiscount)) * currentValue +
          this.dataForm.amount * this.dataForm.rechargeMultiple
      },
      calculation(){
        if(this.dataForm.discountAmount!=null&&this.dataForm.discountMultiple!=null){
          this.codingCoin = Number(this.dataForm.discountAmount)  * this.dataForm.discountMultiple
        }else{
          this.codingCoin =0
        }
      ...
      dataFormSubmit () {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            let v = this.dataForm.rechargeType.split(',')
            var rechargeType = v[0]
            var rechargeName = v[1]
            var confimMessage = ''
            var remark = ''
            var operation = ''
            var disAmount=this.dataForm.discountAmount
            if (this.dataForm.operationType == 0) {
              operation = '人工存款'
              remark = `为会员账号【${this.dataForm.userAccount}】${operation}${rechargeName}【${this.dataForm.discountAmount}】`

              if (this.dataForm.rechargeType.indexOf('0,') > -1) {
                if(!this.disSwitch){
                  disAmount=0
                }
