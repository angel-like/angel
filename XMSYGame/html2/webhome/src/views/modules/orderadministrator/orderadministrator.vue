<template>
	<div class="mod-config">
        ...
        <el-select v-model="dataForm.rechargeType" placeholder="充值类型" clearable>
          <el-option v-for="item in typeOptions" :key="item.id" :label="item.name" :value="item.code">
			...
			<el-form-item>
				...
				<el-button v-if="isAuth('orderadministratorrecharge:orderadministratorrecharge:accessMoney')" type="primary" @click="createHandle()">创建存款订单</el-button>
				<el-button @click="exportCSV()">下载Excel</el-button>
				<el-button v-if="isAuth('orderadministratorrecharge:orderadministratorrecharge:delete')" type="danger" @click="deleteHandle()"
      ...
      <el-table-column prop="rechargeTypeName" header-align="center" align="center" label="充值类型">
      </el-table-column>
...
<script>
	...
	import create from './orderadministrator-create'
	...
	export default {
      ...
      this.dataForm.rechargeType=this.$route.query.rechargeType
			this.getDataList()
		},
		created() {
			this.keyupSubmit()
